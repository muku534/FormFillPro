...
const FieldDetector = {
  patterns: {
    ...
    workExpLocation: { attributes: ['job-location', 'work-location', 'employment-location', 'office-location', 'work-city'], validFor: ['input'], autocomplete: [], isWorkExp: true },
    workExpStartDate: { attributes: ['start-date', 'startdate', 'from-date', 'fromdate', 'begin-date', 'begindate', 'date-from', 'datefrom', 'employment-start', 'work-start'], validFor: ['input'], autocomplete: [], isWorkExp: true },
    ...
    workExpDescription: { attributes: ['job-description', 'role-description', 'work-description', 'responsibilities', 'duties', 'job-duties', 'role-summary', 'work-summary', 'accomplishments', 'achievements'], validFor: ['textarea', 'input'], autocomplete: [], isWorkExp: true },
...
const FormFiller = {
  ...
  async fillAllForms(profile, options = {}) {
    ...
    for (let i = 0; i < allFields.length; i++) {
      ...
      if (applicationQuestions && typeof ApplicationQuestionsFiller !== 'undefined') {
        const detection = ApplicationQuestionsFiller.detectAndFill(field, applicationQuestions);
        if (detection) {
          ApplicationQuestionsFiller.fillApplicationQuestion(element, detection.answer, field.options);
          filledCount++;
          continue;
        }
      }
...
const ApplicationQuestionsFiller = {
  detectAndFill(field, applicationQuestions) {
    if (!applicationQuestions || Object.keys(applicationQuestions).length === 0) {
      return null;
    }

    if (typeof ApplicationQuestions === 'undefined') {
      return null;
    }
    ...
    const detection = ApplicationQuestions.detectQuestion(labelText, options);

    if (!detection || detection.confidence < 0.5) {
      return null;
    }

    const answer = ApplicationQuestions.getAnswer(detection.questionKey, applicationQuestions);
    if (!answer) {
      return null;
    }
  ...
  fillApplicationQuestion(element, answer, options = []) {
    const tagName = element.tagName.toLowerCase();
    const inputType = element.type?.toLowerCase();

    if (tagName === 'select') {
      const selectOptions = Array.from(element.options);
      const matchedValue = this.matchSelectOption(selectOptions, answer);
    ...
    if (inputType === 'radio') {
      ...
      if (name) {
        const radioGroup = document.querySelectorAll(`input[type="radio"][name="${name}"]`);
        const matched = this.matchRadioOption(radioGroup, answer);
        if (matched) {
          FormFiller.setRadioValue(radioGroup, matched);
          return true;
        }
      }
      return false;
    ...
    if (inputType === 'checkbox') {
      const shouldCheck = this.isAffirmativeAnswer(answer);
      FormFiller.setCheckboxValue(element, shouldCheck);
      return true;
    }

    FormFiller.setFieldValue(element, answer);
    return true;
  },
  ...
  matchSelectOption(options, answer) {
    ...
    const noPatterns = ['no', 'false', 'n', 'i do not', 'i am not', 'negative'];

    const isYesAnswer = yesPatterns.some(p => answerLower.includes(p) || answerLower === p);
    const isNoAnswer = noPatterns.some(p => answerLower === p || answerLower.startsWith(p + ' ') || answerLower.startsWith(p + ','));

    if (isYesAnswer) {
      const yesOption = options.find(opt => {
        const optText = opt.text.toLowerCase().trim();
        return yesPatterns.some(p => optText.startsWith(p) || optText === 'yes');
      });
    ...
    const preferNotPatterns = ['prefer not', 'decline', 'rather not'];
    const isPreferNot = preferNotPatterns.some(p => answerLower.includes(p));
    if (isPreferNot) {
      const preferNotOption = options.find(opt => {
        const optText = opt.text.toLowerCase();
        return preferNotPatterns.some(p => optText.includes(p));
      });
      if (preferNotOption) return preferNotOption.value;
    }
  ...
  matchRadioOption(radioGroup, answer) {
    const radios = Array.from(radioGroup);
    const answerLower = answer.toLowerCase().trim();

    for (const radio of radios) {
      const label = this.getRadioLabel(radio);
      if (label && label.toLowerCase().trim() === answerLower) {
        return radio.value;
      }
      if (radio.value.toLowerCase() === answerLower) {
        return radio.value;
      }
    }

    const yesPatterns = ['yes', 'true', 'y', 'i do', 'i am'];
    const noPatterns = ['no', 'false', 'n'];

    const isYesAnswer = yesPatterns.some(p => answerLower.includes(p));
    const isNoAnswer = noPatterns.some(p => answerLower === p);

    for (const radio of radios) {
      const label = (this.getRadioLabel(radio) || radio.value || '').toLowerCase();
      if (isYesAnswer && yesPatterns.some(p => label.startsWith(p))) {
        return radio.value;
      }
      if (isNoAnswer && noPatterns.some(p => label === p || label.startsWith(p + ' '))) {
        return radio.value;
      }
    }
  ...
  isAffirmativeAnswer(answer) {
    const yesPatterns = ['yes', 'true', 'y', '1', 'i do', 'i am', 'i have', 'agree', 'authorize', 'consent'];
    const answerLower = answer.toLowerCase().trim();
